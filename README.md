...
│
├── mcp-server/                 MCP 서버 (Claude / Cursor 연동)
│   └── dist/index.js           번들된 ESM — fill_hwpx, extract_placeholders, list_templates
│
...
└── web/                        Next.js 웹 애플리케이션
    ...
    ├── src/app/demo/           플레이그라운드 UI (인증 불필요)
...
표준 라이브러리만 사용하는 순수 Python 패키지입니다. 외부 의존성 없음.
...
fill_template(
    ...
    data={"TITLE": "2025 연간 보고서", "AUTHOR": "홍길동"},
    output_path="output.hwpx",
)

# 텍스트 노드 추출
nodes = extract_nodes("document.hwpx")
# → [{"file": "Contents/content0.xml", "index": 0, "text": "제목 입력"}, ...]
```
...
Claude Desktop 또는 Cursor에서 HWPX 작업을 자연어로 처리합니다.

**설정 추가 (`claude_desktop_config.json` 또는 `.cursor/mcp.json`):**
...
**제공 도구:**
- `fill_hwpx` — 템플릿의 `{{PLACEHOLDER}}`를 JSON 데이터로 치환
- `extract_placeholders` — HWPX에서 `{{PLACEHOLDER}}` 목록 추출
- `list_templates` — 서버에 등록된 템플릿 목록 조회

---

### 플레이그라운드 — 브라우저에서 바로 체험 (인증 불필요)

회원가입·로그인 없이 HWPX 기능을 즉시 체험할 수 있는 데모 페이지입니다.
...
**동작 방식:**
1. 서버에 내장된 샘플 템플릿 선택
2. `{{PLACEHOLDER}}` 필드 자동 감지 → 입력 폼 생성
3. 값 입력 후 완성된 HWPX 파일 즉시 다운로드
...
| 파이프라인에 통합 | pyhwpx 패키지 | `pip install pyhwpx` |
| Claude / Cursor에서 직접 사용 | MCP 서버 | `mcp-server/dist/index.js` |
| 브라우저에서 즉시 체험 | 플레이그라운드 | `/demo` (인증 불필요) |
...
서버에 내장된 샘플 템플릿 목록과 각 템플릿의 플레이스홀더를 반환합니다.

```bash
curl https://YOUR_DOMAIN/api/public/templates
# → {"templates":[{"id":"annual-report","name":"연간 보고서","placeholders":["TITLE","AUTHOR","SUMMARY"]},...]}
```

### `GET /api/public/docs`

OpenAPI 3.0.3 스펙 JSON. 서드파티 도구에서 API를 자동으로 파싱할 때 사용합니다.
...
Swagger UI 인터페이스. 브라우저에서 API를 직접 탐색하고 호출할 수 있습니다.
...
| `pyhwpx` 패키지 | Python 3.8+ (표준 라이브러리만) |
| MCP 서버 | Node.js 18+ |
| 웹 UI | Node.js 18+, npm |
...
- CI 기반 문서 생성은 `.github/workflows/examples/hwpx-ci-example.yml` 참고
...
**MCP 서버를 빌드하려면?**
`cd mcp-server && npm install && npm run build` — `dist/index.js`가 생성됩니다.

**pyhwpx를 pip 없이 사용할 수 있나요?**
`pyhwpx/` 디렉터리를 그대로 복사해서 `from pyhwpx import fill_template`으로 임포트할 수 있습니다. 표준 라이브러리만 사용합니다.
