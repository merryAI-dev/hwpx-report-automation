...
import { parseHwpxTemplate, buildFieldCoordMap, type TemplateField } from "@/lib/batch/hwpx-template-parser";
...
export default function BatchPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="mb-1 text-xl font-semibold text-[var(--color-notion-text)]">
        일괄 HWPX 생성
      </h1>
      <p className="mb-8 text-sm text-[var(--color-notion-text-secondary)]">
      ...
      <div className="mt-8 space-y-6">
        ...
        {(step === "upload" || step === "mapping" || step === "options") && (
          <Section title="1. 파일 선택">
            ...
            {pipelineType === "coordinate" && templateFields.length > 0 && (
              <div className="mt-3 rounded-md bg-[var(--color-notion-bg-hover)] p-3">
            ...
            <input ref={tplInputRef} type="file" accept=".hwpx" className="hidden"
              ...
              <>
                ...
                <div className="space-y-3">
                  {templateFields.map((field) => {
                    ...
                    return (
                      <div key={`${field.inputCell.col}-${field.inputCell.row}`} className="flex items-center gap-3">
        ...
        {step === "options" && (
          <Section title="3. 생성 옵션">
            {pipelineType === "placeholder" ? (
              /* 플레이스홀더 모드: 심플 직접 치환만 */
              <div className="rounded-lg border-2 border-[var(--color-notion-accent)] bg-[var(--color-notion-accent-light)] p-4">
        ...
        {step === "processing" && (
          <Section title="처리 중...">
            <div className="space-y-4 py-4">
              <div className="h-2 overflow-hidden rounded-full bg-[var(--color-notion-bg-hover)]">
                <div
                  className="h-full rounded-full bg-[var(--color-notion-accent)] transition-all duration-300"
                  style={{
              ...
              <p className="text-center text-sm text-[var(--color-notion-text-secondary)]">
                {progress.total > 0
        ...
        return (
          <div key={i} className="flex items-center">
            <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold
              ${done ? "bg-[var(--color-notion-green)] text-white"
                : active ? "bg-[var(--color-notion-accent)] text-white"
                : "bg-[var(--color-notion-bg-hover)] text-[var(--color-notion-text-tertiary)]"}`}>
              {done ? "✓" : i + 1}
            </div>
            <span className={`ml-2 mr-4 hidden text-xs sm:inline
              ${active ? "font-medium text-[var(--color-notion-text)]"
                : "text-[var(--color-notion-text-secondary)]"}`}>
              {s.label}
            </span>
            {i < steps.length - 1 && (
              <div className={`mr-4 h-px w-6 ${done ? "bg-[var(--color-notion-green)]" : "bg-[var(--color-notion-border)]"}`} />
            )}
          </div>
        );
      })}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-[var(--color-notion-border)] bg-white p-6">
      <h2 className="mb-4 text-sm font-semibold text-[var(--color-notion-text)]">{title}</h2>
      {children}
    </div>
  );
}
...
function DropZone({
  label, accept, file, hint, hintColor, onDrop, onClick,
}: {
  ...
  return (
    <div
      className={`relative flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed p-6 text-center transition-colors
        ${dragging ? "border-[var(--color-notion-accent)] bg-[var(--color-notion-accent-light)]"
          : file ? "border-[var(--color-notion-green)] bg-green-50"
          : "border-[var(--color-notion-border)] hover:border-[var(--color-notion-border-strong)] hover:bg-[var(--color-notion-bg-hover)]"}`}
    ...
    >
      ...
      <p className="text-sm font-medium text-[var(--color-notion-text)]">{label}</p>
      {file ? (
        <p className="text-xs text-[var(--color-notion-green)] font-medium">{file.name}</p>
      ) : (
        <p className="text-xs text-[var(--color-notion-text-secondary)]">클릭 또는 드래그</p>
      )}
      <p className={`text-xs ${
        hintColor === "red" ? "text-red-500"
        : hintColor === "green" ? "text-[var(--color-notion-green)]"
        : hintColor === "blue" ? "text-blue-600"
        : "text-[var(--color-notion-text-tertiary)]"
      }`}>
        {hint}
      </p>
    </div>
...
function ModeCard({
  value, selected, onSelect, title, badge, badgeColor, desc, pros, cons,
}: {
  value: BatchMode; selected: boolean; onSelect: () => void;
    ...
    >
      <div className="flex items-center gap-2">
        <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center shrink-0
          ${selected ? "border-[var(--color-notion-accent)]" : "border-[var(--color-notion-border-strong)]"}`}>
          {selected && <div className="h-2 w-2 rounded-full bg-[var(--color-notion-accent)]" />}
        </div>
        <span className="font-medium text-sm text-[var(--color-notion-text)]">{title}</span>
      ...
      <p className="mt-2 text-xs text-[var(--color-notion-text-secondary)] ml-6">{desc}</p>
      <div className="mt-3 ml-6 grid grid-cols-2 gap-2">
        <div>
          {pros.map((p) => (
            <p key={p} className="text-xs text-green-700 before:content-['✓_']">{p}</p>
          ))}
        </div>
        <div>
          {cons.map((c) => (
            <p key={c} className="text-xs text-[var(--color-notion-text-secondary)] before:content-['·_']">{c}</p>
          ))}
        </div>
      </div>
    </button>
