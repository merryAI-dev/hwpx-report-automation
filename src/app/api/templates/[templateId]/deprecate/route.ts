...
import { deprecateWorkspaceTemplate } from "@/lib/server/workspace-store";
...
export const POST = withApiAuth(async (
  ...
  try {
    ...
    const template = await deprecateWorkspaceTemplate({ tenantId: actor.tenantId, actor, templateId });
    if (!template) {
      return NextResponse.json({ error: "Template not found." }, { status: 404 });
    }
    return NextResponse.json({ template: attachWorkspaceTemplateDownloads(actor.tenantId, template) });
  } catch (error) {
    return workspaceErrorResponse(error, "Failed to deprecate template.");
  }
}, { requireTenant: true });
